...
//TODO Add the fade delay
bool LED::FadeStatic(int LEDArray[],byte start_color[], byte end_color[])
{
	static int i = 0;
	int change_color_red = end_color[0] - start_color[0];
	int change_color_green = end_color[1] - start_color[1];
...
void LED::AddColor(byte color[]){
	int i;

	for(i=0; i<3; i++){
		colorCircular[pos][i] = color[i];
	}

	n++;
}

void LED::UpdateFadeStaticCircular(int LEDArray[]){
	...
	if(n>0){
		...
		posTemp = pos+1;
		if(posTemp==MAX_COLORS)
			posTemp=0;

		if(FadeStatic(LEDArray, colorCircular[pos], colorCircular[posTemp]) == false){
			pos++;
			if(pos==MAX_COLORS)
				pos=0;
			n--;
